    ...
    public void damageMonster(int i, Entity attacker, int attack, int knockBackPower ) {
	   if(i != 999) {
		  if(!gp.monster[gp.currentMap][i].invincible) {
			  ...
			  if(gp.monster[gp.currentMap][i].offBalance) {//This is the parry attack increase 
				System.out.println(gp.bossBattleOn);  
				if(!gp.bossBattleOn) {
					attack = (int)(getAttack() * 2.5);
				  }else { 
					  attack = (int)(getAttack() * 1.25);
				  }
